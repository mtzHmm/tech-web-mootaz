...
export class Library {
    ...
    constructor() {
        ...
        this.transactionRepository = new Repository();
     ...
     */
    addBook(title, author, year, isbn, category) {
     ...
     */
    searchBooks(query) {
        const results = this.bookRepository.findAll(book => book.title.toLowerCase().includes(query.toLowerCase()) ||
     ...
     */
    addAdmin(name, permissions = [], email) {
        const id = this.generateUserId();
    ...
    returnBook(userId, bookId) {
        ...
        const transaction = this.transactionRepository.find(t => t.userId === userId && t.bookId === bookId && !t.returnDate);
        if (transaction) {
    ...
    async loadBooksFromApi() {
        try {
            const response = await this.apiService.fetchBooks();
            if (response.success) {
                response.data.forEach(bookData => {
                    try {
                        const book = new BookImpl(bookData.id, bookData.title, bookData.author, bookData.year, bookData.isbn, bookData.category);
